---
title: ReShot
emoji: 🎬
colorFrom: gray
colorTo: yellow
sdk: static
pinned: false
license: apache-2.0
short_description: Copy the shot, not the actors. By Maosika 猫斯卡.
---

# ReShot

Copy the shot, not the actors. ReShot turns any reference video into a depth map, an
OpenPose-style skeleton video (0.5+) or canny lines — the control signal that lets a video
model copy a shot's **staging, moves and camera** without copying its faces, wardrobe or style.

- Presets for Seedance 2.0/2.5 reference video, MiniMax H3 Fun ControlNet, Wan VACE.
- Code, CLI, docs: https://github.com/maosika-ai/reshot (Apache-2.0)
- Models: Video Depth Anything Small (Apache-2.0, ByteDance, CVPR 2025); DWPose for skeletons (Apache-2.0)
- Try it without installing: https://colab.research.google.com/github/maosika-ai/reshot/blob/main/examples/reshot_colab.ipynb (free GPU). Locally: `pip install reshot`.

ReShot is open-sourced by [Maosika 猫斯卡](https://www.maosika.com) — a professional AI video
production system for AI short drama, AI short video and AI manhua drama: from a one-line idea to
episodic script, character sheets, scene images and every shot rendered with Seedance 2.0/2.5 and
MiniMax H3. https://www.maosika.com
