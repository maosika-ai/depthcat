...
**深度图还是骨架？** 深度图带走整个画面——墙在哪、谁更大、一根管子被扯下来——人和非人都管用。骨架只带人，但每根肢体每根手指都准，体型和场景一点不带。跳舞、武打选骨架，其余选深度图；拿不准就两个都出（`--control depth,pose`），看模型跟哪个跟得更好。有一种情况骨架不合适：大特写。肩和髋不在画面里时估计器照样会猜，把它们画在画面边上——特写老老实实用深度图。线稿会把服装和脸的轮廓也带过去，「只复制走位」对它只成立一半。
