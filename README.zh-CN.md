...
| | 能不能跑 | 实测 |
|---|---|---|
| **N 卡 12 GB 及以上**（RTX 3080 Ti、4070、4090……） | 能，`--quality full` | 4090：62 ms/帧、显存峰值 11 GB；3080 Ti：83 ms/帧 |
| **N 卡 8 GB**（RTX 3070、4060……） | 能，`--quality fast`（自动选） | 显存 3 GB、3080 Ti 上 34 ms/帧 |
...
**`--quality` 由你定**：`auto`（默认）在 11.5 GB 以上的卡取 `full`、以下取 `fast`；想自己定就直接传 `full` 或 `fast`。两者的差别（同一段 294 帧的片实测）：模型看到的画面短边 518 像素还是 364 像素；大形状——人、道具、谁近谁远——两边一模一样，细的东西 `fast` 会软（脸颊旁一缕头发会糊进脸里）。平均差 5.3 个灰阶（满量程 255），95% 的像素差在 15 以内，边缘锐度低 4.5%。复制走位和运镜，`fast` 够用；特写里在意细轮廓，用 `full`。要精确尺寸用 `--input-size`。
