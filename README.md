...
depthcat turns a reference clip into a depth blockout: a clean, silent record of the
...
depthcat reference.mp4 -o blockout.mp4 --target seedance
```
...
A blockout is only useful if the model trusts it. So depthcat is built around the details
...
depthcat is Apache-2.0, and so is the model it runs by default — Video Depth Anything
Small, from ByteDance (CVPR 2025), vendored under `depthcat/third_party/`. Use it in a
...
```bash
pip install git+https://github.com/maosika-ai/depthcat          # ffmpeg on PATH
pip install "depthcat[ffmpeg] @ git+https://github.com/maosika-ai/depthcat"   # or bundle one
depthcat --help
```
...
<p align="center"><sub>Made by <a href="https://www.maosika.com">Maoska 猫斯卡</a>, a studio that ships AI short drama every day.</sub></p>
